...
const PublicPetProfile = ({ pet, owner }: { pet: Pet, owner: UserProfile }) => {
    ...
    // ---------------------------------------------------------
    // DISEÑO A: MODO SEGURO (Solo información básica)
    // ---------------------------------------------------------
    if (isSafe) {
        return (
            <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 relative overflow-hidden">
                ...
                <div className="w-full max-w-sm bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden relative z-10 p-6 text-center space-y-6">
                    ...
                    {/* Foto Circular Amigable */}
                    <div className="relative w-40 h-40 mx-auto">
                        <img 
                            src={pet.photoUrl} 
                            className="w-full h-full object-cover rounded-full border-4 border-green-500/30 shadow-lg shadow-green-500/20" 
                            alt={pet.name} 
                        />
                        <div className="absolute bottom-2 right-2 bg-green-500 text-slate-900 p-2 rounded-full border-4 border-slate-900">
                    ...
                    {/* Información Básica */}
                    <div>
                        <h1 className="text-3xl font-black text-white uppercase tracking-wide">{pet.name}</h1>
                        <p className="text-slate-400 font-medium">{pet.breed}</p>
                    </div>

                    {/* Mensaje de Tranquilidad */}
                    <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-700">
                        <p className="text-green-400 font-bold text-lg mb-1">¡Estoy seguro!</p>
                        <p className="text-slate-300 text-sm">
                            Actualmente estoy en casa y cuidado por mi dueño, <span className="font-bold text-white">{owner.firstName}</span>.
                        </p>
                    </div>

                    {/* Footer discreto */}
                    <div className="pt-4 border-t border-slate-800">
                         <p className="text-xs text-slate-600">Identidad verificada por DNI-PETS</p>
                    </div>
                </div>
            </div>
        );
    ...
    // ---------------------------------------------------------
    // DISEÑO B: MODO ALERTA (Perdido / Adopción con contacto)
    // ---------------------------------------------------------
    
    // Preparar datos de contacto solo para estos modos
        ...
        <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden relative z-10">
                ...
                <div className="relative h-72">
                    ...
                    {/* Etiqueta de Estado en la Foto */}
                    <div className="absolute top-4 right-4">
                        {isLost ? (
                            <span className="bg-red-600 text-white font-bold px-4 py-1 rounded-full animate-pulse shadow-lg shadow-red-900/50 flex items-center gap-2">
                                <AlertTriangle className="w-4 h-4" /> PERDIDO
                            </span>
                        ) : (
                            <span className="bg-blue-500 text-white font-bold px-4 py-1 rounded-full shadow-lg shadow-blue-900/50 flex items-center gap-2">
                ...
                {/* Caja de Acción / Mensaje */}
                <div className="px-6 py-4">
                    {isLost ? (
                        <div className="bg-red-500/10 border border-red-500/50 rounded-xl p-4">
                            <h3 className="font-bold text-red-500 text-lg mb-1">¡Ayúdame a volver!</h3>
                            <p className="text-sm text-red-300">Estoy perdido. Si me ves o me encontraste, por favor contacta a mi familia urgentemente.</p>
                        </div>
                    ) : (
                        <div className="bg-blue-500/10 border border-blue-500/50 rounded-xl p-4">
                            <h3 className="font-bold text-blue-400 text-lg mb-1">¡Búscame un hogar!</h3>
                            <p className="text-sm text-blue-300">Estoy buscando una nueva familia que me quiera. Contáctanos para adoptarme.</p>
                        </div>
                    )}
                </div>

                {/* Datos de Contacto (SOLO VISIBLE AQUÍ) */}
                <div className="px-6 pb-8 space-y-4">
                    <div className="bg-slate-800/50 p-4 rounded-xl flex items-center gap-4 border border-slate-700">
                        ...
                        <div>
                            <p className="text-xs text-slate-400 uppercase tracking-wider font-bold">Responsable</p>
                            <p className="text-lg font-bold text-slate-100">{owner.firstName || 'Usuario DNI-PETS'}</p>
                        </div>
                    </div>

                    {whatsappLink ? (
                        <a href={whatsappLink} target="_blank" rel="noreferrer" className={`w-full py-4 font-bold rounded-xl flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-lg ${isLost ? 'bg-red-600 hover:bg-red-700 shadow-red-900/20 text-white' : 'bg-[#25D366] hover:bg-[#128C7E] shadow-green-900/20 text-white'}`}>
                            <MessageCircle className="w-6 h-6" /> {isLost ? 'Reportar Hallazgo' : 'Contactar por WhatsApp'}
                        </a>
                    ) : (
